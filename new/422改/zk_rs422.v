...
module zk_rs422(
	...
	output	[15:0] 	O_ChengXiangZengYi,	
...
reg [1:0] send_fsm;
//////////////package/////////////////////////
reg [7:0] send_data[49:0]; //send message 
//reg [1:0] package_fsm;
//reg [5:0] pack_cnt;
...
always @(posedge I_clk) begin
	if (!I_rst) begin
		...
		if (flag_cmd_send == 1'b1) begin  // 收到查询状态包需应答
			...
			if (resp_timeout_cnt >= 18'd160000) begin  // 超过2ms强制触发发送
			...
			if (rx_rdy) begin						//receive one byte
				...
				case (receive_fsm)
                   ...
                   2: begin												//
                    ...
                    3: begin
						if ((rx_count==3) && (rx_data == 8'h07)) begin			//设备标识, LSB   0xFF07为长波相机
							receive_fsm <= 4;
						end //end if
						else begin
							rx_count <= 0;	
					...
					5: begin												//参数包
						if (rx_count == 5) begin							//包长度
							...
							cmd_value_zy[15:8] <= rx_data;
							crc_rcv <= crc_rcv + rx_data;
						end //end if
						else if (rx_count == 11) begin							//成像增益
							cmd_value_zy[7:0] <= rx_data;
							crc_rcv <= crc_rcv + rx_data;
						end //end if
                        else if (rx_count == 12) begin							//工作模式
							cmd_value_gm[7:0] <= rx_data;
							crc_rcv <= crc_rcv + rx_data;
						end //end if
                        else if (rx_count == 13) begin							//合并模式
                        ...
                        else if (rx_count == 15) begin							//行，MSB
							ROW_NUM [7:0] <= rx_data;
                            crc_rcv <= crc_rcv + rx_data;
						end //end if
                        else if (rx_count == 16) begin							//列，MSB
							COLUMN_NUM [15:8] <= rx_data;
                            crc_rcv <= crc_rcv + rx_data;
						end //end if
                        else if (rx_count == 17) begin							//列，LSB
							COLUMN_NUM [7:0] <= rx_data;
                            crc_rcv <= crc_rcv + rx_data;
						end //end if
                        else if (rx_count == 18) begin							//年
							year_tmp[7:0] <= rx_data;
                            crc_rcv <= crc_rcv + rx_data;
						end //end if
						else if (rx_count == 19) begin							//月
							month_tmp[7:0] <= rx_data;
                            crc_rcv <= crc_rcv + rx_data;
						end //end if
						else if (rx_count == 20) begin							//日
							date_tmp[7:0] <= rx_data;
                            crc_rcv <= crc_rcv + rx_data;
						end //end if
                        else if (rx_count == 21) begin							//时
							hour_tmp[7:0] <= rx_data;
                            crc_rcv <= crc_rcv + rx_data;
						end //end if
						else if (rx_count == 22) begin							//分
							minute_tmp[7:0] <= rx_data;
                            crc_rcv <= crc_rcv + rx_data;
						end //end if
                        else if (rx_count == 23) begin							//秒
							second_tmp[7:0] <= rx_data;
                            crc_rcv <= crc_rcv + rx_data;
						end //end if
						else if (rx_count == 24) begin							//毫秒，MSB
							millisecond_tmp[15:8] <= rx_data;
                            crc_rcv <= crc_rcv + rx_data;
						end //end if
						else if (rx_count == 25) begin							//毫秒，LSB
		...
		if (flag_cmd_send == 1'b1) begin
			if (package_type == 8'h03) begin
				...
				else if (tx_count == 10) begin			//成像增益, MSB
					send_data[10] <= ChengXiangZengYi[15:8];
					crc_send <= crc_send + ChengXiangZengYi[15:8];
				end //end else if
				else if (tx_count == 11) begin			//成像增益, LSB
					send_data[11] <= ChengXiangZengYi[7:0];
					crc_send <= crc_send + ChengXiangZengYi[7:0];
				end //end else if
                else if (tx_count == 12) begin			//工作模式
					send_data[12] <= GongZuoMoShi[7:0];
					crc_send <= crc_send + GongZuoMoShi[7:0];
				end //end else if
                else if (tx_count == 13) begin			//合并模式
					send_data[13] <= HeBingMoShi[7:0];
					crc_send <= crc_send + HeBingMoShi[7:0];
				end //end else if
                else if (tx_count == 14) begin			//行，MSB
					send_data[14] <= ROW_NUM [15:8];
					crc_send <= crc_send + ROW_NUM [15:8];
				end //end else if
                else if (tx_count == 15) begin			//行，LSB
					send_data[15] <= ROW_NUM [7:0];
					crc_send <= crc_send + ROW_NUM [7:0];
				end //end else if
                else if (tx_count == 16) begin			//列，MSB
					send_data[16] <= COLUMN_NUM [15:8];
					crc_send <= crc_send + COLUMN_NUM [15:8];
				end //end else if
                else if (tx_count == 17) begin			//列，LSB
					send_data[17] <= COLUMN_NUM [7:0];
					crc_send <= crc_send + COLUMN_NUM [7:0];
				end //end else if
                else if (tx_count == 18) begin			//年
					send_data[18] <= year [7:0];
					crc_send <= crc_send + year [7:0];
				end //end else if 
                else if (tx_count == 19) begin			//月
					send_data[19] <= month [7:0];
					crc_send <= crc_send + month [7:0];
				end //end else if 
                else if (tx_count == 20) begin			//日
					send_data[20] <= date [7:0];
					crc_send <= crc_send + date [7:0];
				end //end else if 
                else if (tx_count == 21) begin			//时
					send_data[21] <= hour [7:0];
					crc_send <= crc_send + hour [7:0];
				end //end else if 
                else if (tx_count == 22) begin			//分
					send_data[22] <= minute [7:0];
					crc_send <= crc_send + minute [7:0];
				end //end else if 
                else if (tx_count == 23) begin			//秒
					send_data[23] <= second_syn [7:0];
					crc_send <= crc_send + second_syn [7:0];
				end //end else if 
                else if (tx_count == 24) begin			//毫秒，MSB
					send_data[24] <= millisecond_syn [15:8];
					crc_send <= crc_send + millisecond_syn [15:8];
				end //end else if 
                else if (tx_count == 25) begin			//毫秒，LSB
					send_data[25] <= millisecond_syn [7:0];
